...
describe('Registrar Conta', () => {
    it.only('Registrar conta', () => {
        ...
        cy.get(':nth-child(3) > #username > .native-input').type('victorgabriel.p07@mailinator.com');
        ...
        cy.wait('@postEnter').then((interception) => {
            expect(interception.status).be.eq(1);
            expect(interception.response.body).has.property('id');
            expect(interception.response.body.id).is.not.null;
